// server.js (FINAL - Clean Source Labels & Advanced Filtering)
...
const filterUbuyFromGoogle = (results) => {
    ...
    const googleShoppingSource = 'google shopping'; // Use the new clean label
...
app.get('/search', async (req, res) => {
    ...
    try {
        ...
        console.log(`Kept ${filteredForUbuy.length} results after removing Google's Ubuy listings.`);
...
async function searchPricerAPI(query) {
    try {
        ...
        return response.data.map(item => ({
            source: 'Pricer', // MODIFIED: Cleaned label
...
async function searchPriceApiCom(query) {
    ...
    try {
        const jobsToSubmit = [
            ...
            { source: 'google shopping', topic: 'search_results', key: 'term', values: query } // MODIFIED: lowercase for consistency
        ];
        ...
        for (const data of downloadedResults) {
            let mapped = [];
            const sourceName = data.source; // MODIFIED: This is now the clean source name
            if (data.topic === 'product_and_offers') {
                const products = data.results?.[0]?.products || [];
                mapped = products.map(item => ({ source: sourceName, title: item?.name, price: item?.price, price_string: item?.offer?.price_string || (item?.price ? `$${item.price.toFixed(2)}` : 'N/A'), url: item?.url, image: item?.image || 'https://via.placeholder.com/150', store: item?.shop?.name || sourceName }));
                ...
                mapped = searchResults.map(item => {
                    ...
                    if (item.name && price !== null) return { source: sourceName, title: item.name, price: price, price_string: price_string, url: item.url, image: item.img_url || 'https://via.placeholder.com/150', store: item.shop_name || sourceName };
                    return null;
                }).filter(Boolean);
            }
