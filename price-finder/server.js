// server.js (FINAL, NO DUPLICATES - Scrape-Lite with API Fallback)
...
async function searchPricerAPI(query) {
    try {
        ...
        return response.data.map(item => ({ source: 'Pricer', title: item?.title || 'Title Not Found', price_string: item?.price || 'N/A', url: item?.url, image: item?.img, store: item?.shop ? item.shop.replace(' from ', '') : 'Seller Not Specified' }));
