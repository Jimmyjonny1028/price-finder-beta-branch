// server.js (FINAL - With Corrected Job Queue and All Features)
...
wss.on('connection', (ws, req) => {
    ...
    if (secret !== SERVER_SIDE_SECRET) {
        console.log("A client tried to connect with the wrong secret.");
        ws.close();
        return;
    }
...
app.get('/search', async (req, res) => {
    ...
    if (workerSocket) {
        // --- THE FIX FOR JOB SPAMMING ---
        const isJobAlreadyQueued = jobQueue.includes(query);
        const isJobRunning = isWorkerBusy && workerSocket.currentJob === query;
        if (!isJobAlreadyQueued && !isJobRunning) {
            jobQueue.push(query);
            console.log(`Query "${query}" added to the job queue. Position: ${jobQueue.length}`);
        } else {
            console.log(`Query "${query}" is already being processed or is in the queue.`);
        }
        triggerNextJob();
