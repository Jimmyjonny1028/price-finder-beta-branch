// server.js (FINAL, STABLE - API-Based Version)
...
async function searchPricerAPI(query) {
    try {
        ...
        return response.data.map(item => ({ title: item?.title || 'Title Not Found', price_string: item?.price || 'N/A', url: cleanGoogleUrl(item?.link), image: item?.img, store: item?.shop ? item.shop.replace(' from ', '') : 'Seller Not Specified' }));
...
app.get('/search', async (req, res) => {
    ...
    console.log(`Starting API-based search for: ${query}`);
    ...
    try {
        ...
        let allResults = [...pricerResults, ...priceApiComResults].map(item => ({ ...item, price: parseFloat(String(item.price_string || item.price).replace(/[^0-9.]/g, '')), condition: detectItemCondition(item.title), image: formatImageUrl(item.image) })).filter(item => !isNaN(item.price));
