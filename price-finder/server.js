...
app.use(express.json({ limit: '10mb' })); // Increased payload limit for scraped data
app.use(cors());
...
app.get('/search', async (req, res) => {
    ...
    try {
        ...
        if (trafficLog.searchHistory.length > MAX_HISTORY) {
            trafficLog.searchHistory.splice(MAX_HISTORY);
        }
    } catch (e) {
        console.error("Error logging traffic:", e);
    }
...
app.post('/submit-results', (req, res) => {
    ...
    let allResults = results.map(item => ({
        ...item,
        price: parseFloat(String(item.price_string || '').replace(/[^0-9.]/g, '')),
