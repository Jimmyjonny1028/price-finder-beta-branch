...
let fullResults = [];
let loadingInterval;
const loadingMessages = [ "Sending request to your personal scraper...", "Searching Google Shopping & eBay...", "Checking Amazon & JB Hi-Fi...", "Scanning other major retailers...", "Analyzing general search results...", "Compiling all the deals...", "This may take a minute...", "Almost finished..." ];
...
async function handleSearch(event) {
    ...
    try {
        ...
        if (results.length > 0) { fullResults = results; populateAndShowControls(); applyFiltersAndSort(); } else { resultsContainer.innerHTML = `<p>No cached results found. Please ensure your local scraper is running and try again.</p>`; }
...
function pollForResults(query, attempt = 1) {
    ...
    if (attempt > maxAttempts) { loader.classList.remove('polling'); loader.classList.add('hidden'); resultsContainer.innerHTML = `<p class="error">The search took too long to complete. Please check your local scraper and try again.</p>`; searchButton.disabled = false; clearInterval(loadingInterval); return; }
